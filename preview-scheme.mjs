// Generates a standalone HTML preview of the DrawerSchemeRenderer
...
// --- SVG generation (mirrors schemeSvg.ts) ---
const BRAND = {
...
function buildSvg(zones, drawer) {
  ...
  const zoneEls = zones.map((z, i) => {
    const px = FRAME_PAD + z.x_cm*SCALE + half;
    const py = FRAME_PAD + z.y_cm*SCALE + half;
    const pw = z.assigned_w_cm*SCALE - ZONE_GAP;
    const ph = z.assigned_d_cm*SCALE - ZONE_GAP;
    const cx = px + pw/2, cy = py + ph/2;
    const bfs = Math.max(7, Math.min(10, ph*0.18));
    const nfs = Math.max(8, Math.min(13, ph*0.22));
    ...
    return `<g class="zone" data-id="${z.zone_id}">
      <rect x="${px}" y="${py}" width="${pw}" height="${ph}"
        fill="${fill}" stroke="${stroke}" stroke-width="1" rx="${ZONE_R}" class="zone-rect"/>
  ...
  return `<svg id="scheme" viewBox="0 0 ${tW} ${tH}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:600px;height:auto">
...
const svg = buildSvg(zones, drawer);
...
const rules = result.scheme.layout_rule_evaluations.filter(r => r.status === "pass");
...
function whyHtml() {
  const items = [
    `Собрана под размеры ${input.drawer_width_cm} × ${input.drawer_depth_cm} × ${input.drawer_height_cm} см`,
    `Учитывает выбранные вещи`,
    `Приоритет — ${input.priority === "convenient" ? "удобно" : input.priority}`,
    ...rules.slice(0,2).map(r => r.message),
  ];
  return items.map(t => `<div class="why-item"><span class="check">✓</span><span>${t}</span></div>`).join("");
}
...
<title>Уместно — превью схемы</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: Georgia, serif; background: ${BRAND.cream}; color: ${BRAND.text}; padding: 32px 24px; }
  h1 { font-size: 22px; font-weight: normal; margin-bottom: 4px; }
  .subtitle { color: ${BRAND.textLight}; font-size: 13px; margin-bottom: 28px; }
  .layout { display: grid; grid-template-columns: 1fr 1fr; gap: 32px; max-width: 960px; }
  @media (max-width: 700px) { .layout { grid-template-columns: 1fr; } }
  .left {}
  .dims { margin-top: 20px; }
  .dims h3 { font-size: 13px; font-weight: 600; margin-bottom: 10px; color: ${BRAND.textLight}; text-transform: uppercase; letter-spacing: .04em; }
  .dim-item { display: flex; align-items: center; gap: 8px; font-size: 13px; margin-bottom: 6px; }
  ...
  .list-item { display: flex; align-items: center; gap: 10px; font-size: 13px; margin-bottom: 8px; }
  ...
  .fold-item { display: flex; gap: 10px; font-size: 13px; margin-bottom: 8px; line-height: 1.4; }
  .fold-icon { color: ${BRAND.textLight}; flex-shrink: 0; }
  .why-item { display: flex; gap: 10px; font-size: 13px; margin-bottom: 8px; }
  .check { color: ${BRAND.sage}; font-weight: bold; flex-shrink: 0; }
  .warn { background: #FEF3EC; border-left: 3px solid ${BRAND.accent}; padding: 10px 14px; border-radius: 6px; font-size: 12px; line-height: 1.5; }
  /* hover on zones */
  .zone { cursor: default; }
...
<body>
<h1>Ваш результат: схема ящика</h1>
<p class="subtitle">Превью — данные из реального расчёта движка</p>
<div class="layout">
  <div class="left">
    ${svg}
    <div class="dims">
  ...
  <div class="right">
    <div class="card">
      <h3>Что где хранить</h3>
      ${whatWhereHtml()}
    </div>
    <div class="card">
      <h3>Как сложить вещи</h3>
      ${foldingHtml()}
    </div>
    <div class="card">
      <h3>Почему схема подходит</h3>
      ${whyHtml()}
    </div>
