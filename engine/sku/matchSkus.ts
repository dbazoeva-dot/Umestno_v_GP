  ...
  return schemePayload.assigned_zones.map((zone) => {
    ...
    const candidates = skuCatalog
      .filter((sku) =>
        ...
        sku.capacity_units != null && sku.capacity_units >= lane.cap_per_lane &&
        sku.width_cm != null && sku.width_cm <= lane.w_cm &&
        sku.depth_cm != null && sku.depth_cm <= lane.d_cm &&
        sku.height_cm != null && sku.height_cm <= lane.h_cm)
      .sort((a, b) =>
        (Number(b.color_group === colorPreference) - Number(a.color_group === colorPreference)) ||
        (((b.width_cm ?? 0) * (b.depth_cm ?? 0)) - ((a.width_cm ?? 0) * (a.depth_cm ?? 0))));
    return {
