...
## Адаптивные ячейки — `can_adapt` (соты)

Соты/настраиваемые ячейки — это **`division_type = cells` + `can_adapt = "yes"`**,
...
- `division_type = cells`, `can_adapt = yes`;
...
> Текущий каталог содержит соты как 3 строки `dividers` с фикс-конфигами
> (6×3 / 9×2 / 18×1) — это хак под «движок сам выберет конфиг». Заменить на
> **одну** строку `cells + can_adapt`. (Прямые разделители без ячейки —
