...
const scenario5Debug = scenario5Output.debug as { fit_result: { fit_status: string; failed_dimension?: string; failed_zones: Array<{ content_type: string }> }; adjustment_result?: { adjustment_type?: string }; adjustment_attempts: Array<{ content_type?: string; selected_adjustment_candidate?: string; accepted?: boolean; adjusted_zone_was_failed?: boolean; original_failed_dimension?: string; reason_rejected?: string }>; scheme_payload: { selected_calculated_zones: Array<{ content_type: string; division_type: string }> } };
assert(scenario5Debug.fit_result.fit_status === "fit_partial", "scenario 5 fallback regression: final status should remain fit_partial");
assert(scenario5Debug.fit_result.failed_dimension === "height", "scenario 5 fallback regression: bras height should remain the true blocker");
assert(scenario5Debug.fit_result.failed_zones.some((zone) => zone.content_type === "bras"), "scenario 5 fallback regression: bras should be reported as failed");
assert(scenario5Debug.scheme_payload.selected_calculated_zones.some((zone) => zone.content_type === "socks_regular" && zone.division_type === "cells"), "scenario 5 fallback regression: unrelated socks should remain in cells");
assert(!scenario5Debug.scheme_payload.selected_calculated_zones.some((zone) => zone.content_type === "socks_regular" && zone.division_type === "open"), "scenario 5 fallback regression: unrelated socks must not be converted to open");
assert(scenario5Debug.adjustment_result?.adjustment_type === "no_adjustment", "scenario 5 fallback regression: rejected fallback attempts should be debugged without applying adjustment");
assert(scenario5Debug.adjustment_attempts.some((attempt) => attempt.content_type === "bras" && attempt.accepted === false && attempt.adjusted_zone_was_failed === true && attempt.original_failed_dimension === "height"), "scenario 5 fallback regression: debug should show rejected bras fallback with original failed dimension");
assert(!scenario5Debug.adjustment_attempts.some((attempt) => attempt.content_type === "socks_regular" || attempt.selected_adjustment_candidate === "socks_regular"), "scenario 5 fallback regression: unrelated socks fallback should not be attempted");
...
assert(scenario6Debug.fit_result.fit_status === "fit_partial", "scenario 6 fallback regression: final status should remain fit_partial");
assert(scenario6Debug.fit_result.failed_dimension === "height", "scenario 6 fallback regression: clothing height should remain the true blocker");
assert(scenario6Debug.fit_result.failed_zones.some((zone) => zone.content_type === "jeans"), "scenario 6 fallback regression: jeans should be reported as failed");
assert(scenario6Debug.fit_result.failed_zones.some((zone) => zone.content_type === "sweaters"), "scenario 6 fallback regression: sweaters should be reported as failed");
assert(scenario6Debug.scheme_payload.selected_calculated_zones.some((zone) => zone.content_type === "socks_regular" && zone.division_type === "cells"), "scenario 6 fallback regression: unrelated socks should remain in cells");
assert(!scenario6Debug.scheme_payload.selected_calculated_zones.some((zone) => zone.content_type === "socks_regular" && zone.division_type === "open"), "scenario 6 fallback regression: unrelated socks must not be converted to open");
assert(scenario6Debug.adjustment_result?.adjustment_type === "no_adjustment", "scenario 6 fallback regression: rejected fallback attempts should be debugged without applying adjustment");
assert(scenario6Debug.adjustment_attempts.every((attempt) => attempt.adjusted_zone_was_failed === true && attempt.accepted === false && attempt.original_failed_dimension === "height"), "scenario 6 fallback regression: every fallback attempt should target an original failed zone and be rejected");
assert(!scenario6Debug.adjustment_attempts.some((attempt) => attempt.content_type === "socks_regular" || attempt.selected_adjustment_candidate === "socks_regular"), "scenario 6 fallback regression: unrelated socks fallback should not be attempted");

console.log("ok 3 calibration case(s), 1 validation case, 1 library coverage audit, 2 fallback regression scenarios");
