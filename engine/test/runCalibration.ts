...
const runtimeLayoutRuleIds = new Set(defaultLibraries.layoutRules.layout_rule_definitions?.map((rule) => rule.rule_id));
for (const ruleId of ["ZONE_INTEGRITY", "D01", "D02", "D03", "D04", "D04b", "D05", "D06", "D07", "D08", "D09", "D10"]) {
  assert(runtimeLayoutRuleIds.has(ruleId), `${ruleId}: structured layout rule definition should be present`);
}
const runtimeFallbackRuleIds = new Set(defaultLibraries.layoutRules.fallback_rule_definitions?.map((rule) => rule.rule_id));
for (const ruleId of ["FB01", "FB02", "FB03", "FB04", "FB05"]) {
  assert(runtimeFallbackRuleIds.has(ruleId), `${ruleId}: structured fallback rule definition should be present`);
}

function ruleStatus(output: { scheme_payload?: { layout_rule_evaluations: Array<{ rule_id: string; status: string }> } | null }, ruleId: string) {
  return output.scheme_payload?.layout_rule_evaluations.find((rule) => rule.rule_id === ruleId)?.status;
}

const scenario2RuleOutput = runUmestnoEngine({ drawer_width_cm: 120, drawer_depth_cm: 50, drawer_height_cm: 20, storage_category: "underwear", items: [ { content_type: "socks_regular", volume_level: "large" }, { content_type: "panties", volume_level: "large" }, { content_type: "bras", volume_level: "medium" } ], priority: "convenient" });
const scenario2RuleDebug = scenario2RuleOutput.debug as { layout_rule_evaluations: Array<{ rule_id: string; status: string; evidence: Record<string, unknown> }>; scheme_payload: { fit_status: string; assigned_zones: Array<{ content_type: string; x_cm: number; assigned_w_cm: number }> } };
assert(scenario2RuleDebug.scheme_payload.fit_status === "fit_all", "scenario 2 rule eval: final status should remain fit_all");
assert(ruleStatus(scenario2RuleOutput, "D01") === "ok", "scenario 2 rule eval: D01 should use semantic group adjacency and pass");
assert(ruleStatus(scenario2RuleOutput, "D02") === "violated", "scenario 2 rule eval: D02 should be violated when socks are between bras and panties");
assert(ruleStatus(scenario2RuleOutput, "D05") === "ok", "scenario 2 rule eval: D05 should pass when socks/panties cells are adjacent");
assert(ruleStatus(scenario2RuleOutput, "D06") === "ok", "scenario 2 rule eval: D06 should pass when bras touches drawer edge");
assert(scenario2RuleDebug.layout_rule_evaluations.some((rule) => rule.rule_id === "D02" && rule.status === "violated"), "scenario 2 rule eval: debug should expose D02 violation");

const scenario4RuleOutput = runUmestnoEngine({ drawer_width_cm: 90, drawer_depth_cm: 45, drawer_height_cm: 15, storage_category: "mixed", items: [ { content_type: "tshirts", volume_level: "medium" }, { content_type: "belts", volume_level: "small" }, { content_type: "scarves", volume_level: "small" } ], priority: "convenient" });
const scenario4RuleDebug = scenario4RuleOutput.debug as { adjustment_result?: { adjustment_type?: string } | null; scheme_payload: { fit_status: string; assigned_zones: Array<{ content_type: string; division_type: string }> } };
assert(scenario4RuleDebug.scheme_payload.fit_status === "fit_all", "scenario 4 rule eval: valid fallback behavior should remain fit_all");
assert(scenario4RuleDebug.adjustment_result?.adjustment_type === "open_fallback", "scenario 4 rule eval: tshirts depth fallback should still use open_fallback");
assert(ruleStatus(scenario4RuleOutput, "D01") === "ok", "scenario 4 rule eval: accessories semantic group should be adjacent");
assert(ruleStatus(scenario4RuleOutput, "D05") === "ok", "scenario 4 rule eval: belts/scarves cells should be adjacent");
assert(["ok", "not_applicable", "violated"].includes(ruleStatus(scenario4RuleOutput, "D04") ?? ""), "scenario 4 rule eval: D04 should return a structured status");
assert(ruleStatus(scenario4RuleOutput, "D04b") === "opportunity", "scenario 4 rule eval: D04b should report depth absorption opportunity without changing placement");
...
assert(ruleStatus(scenario7Output, "ZONE_INTEGRITY") === "ok", "scenario 7 rule eval: slots_multi_lane_auto should preserve zone integrity");
assert(ruleStatus(scenario7Output, "D06") === "ok", "scenario 7 rule eval: bras should satisfy drawer-edge D06 check");
assert(ruleStatus(scenario7Output, "D05") === "violated", "scenario 7 rule eval: separated socks/panties cells should violate D05");
assert(scenario7Debug.scheme_payload.selected_calculated_zones.some((zone) => zone.content_type === "bras" && zone.division_type === "slots" && zone.option_id === "slots_multi_lane_auto"), "scenario 7 rule eval: placement behavior should remain slots_multi_lane_auto");

console.log("ok 3 calibration case(s), 1 validation case, 1 library coverage audit, 6 soft height scenarios, 3 layout rule scenarios");
