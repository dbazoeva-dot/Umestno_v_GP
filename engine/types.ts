...
export interface SkuCatalogRow { sku_id: string; division_type: DivisionType; rigidity: string; width_cm: number; depth_cm: number; height_cm: number; capacity_units: number; set_quantity?: number; cols?: number; rows?: number; cell_width_cm?: number; cell_depth_cm?: number; can_rotate?: "yes" | "no"; /** каталожный флаг: настраиваемые ячейки/соты — раскладка cols×rows свободна (см. adaptiveCellsFit) */ adjustable?: "yes" | "no"; color_group?: string; availability_status: string; product_title: string; product_url?: string; price?: number }
