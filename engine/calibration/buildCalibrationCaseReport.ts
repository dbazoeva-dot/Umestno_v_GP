...
export function buildCalibrationCaseReport({ input, schemeId, maxItemsOverride }: { input: UserInput; schemeId: string; maxItemsOverride?: number }) {
  ...
  const layoutRuleEvaluations = evaluatePlacementRules({ drawerSize: normalizedInput.drawerSize, fitResult: finalFitResult, storageUnitProfile: defaultLibraries.storageUnitProfile, layoutRuleDefinitions: defaultLibraries.layoutRules.layout_rule_definitions });
