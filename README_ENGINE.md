...
### BL-14: YooKassa embedded widget вместо redirect (UX-улучшение)

**Status:** open — отложено до набора первой статистики конверсии.

**Контекст:** на MVP подключается через redirect-flow (юзер
кликает «Оплатить» → редирект на хостинг ЮКассы → оплата → редирект
обратно на /result/). Это просто и стандартно, но юзер уходит
со страницы на ~30 секунд, что может ощущаться как «прыгнул куда-то».

**Альтернатива (BL-14):** заменить redirect на embedded widget
ЮКассы (`https://yookassa.ru/checkout-widget/v1/checkout-widget.js`).
Сервер создаёт платёж с `confirmation: { type: "embedded" }`, получает
`confirmation_token`, фронт показывает виджет внутри нашей страницы
(модалка или встроенная форма с полями карты). Юзер не покидает сайт.

**Что нужно:**
1. Фронт: подгрузка скрипта widget, инициализация с `confirmation_token`,
   обработка callback'ов (succeeded / pending / error).
2. Стайлинг чтобы виджет не выглядел инородно на странице.
3. Мобильная адаптация модалки (часто это узкое место виджет-флоу).
4. Тестирование на разных браузерах + iOS Safari.

**Когда делать:**
- После первых ~100 реальных платежей через redirect-flow.
- Если в Метрике видно что > 15% юзеров отваливаются на шаге
  «прыгнул на ЮКассу» (не возвращаются), embedded-widget может
  улучшить конверсию.
- Если конверсия и так норм — оставляем redirect, виджет не приоритет.

**Зависимости:** YooKassa-интеграция (redirect-flow) должна работать
и иметь статистику. То есть BL-14 — это **оптимизация**, не базовая
функциональность.

### BL-15: Single source of truth для цены Услуги

**Status:** open — технический долг, появился при выводе paywall в бой.

**Контекст:** на MVP цена Услуги (149 ₽) живёт **в 7 местах**:

1. `.env` → `PRICE_KOP=14900` — фактический источник, уходит в ЮКассу.
2. `oferta/index.html` п. 5.1 — «составляет 149 рублей» (юридически
   подстрахованы формулировкой «На дату публикации настоящей редакции»).
3. `index.html` (лендинг) — 5 мест: meta-description + 4 CTA-кнопки
   («Начать расчёт — 149 ₽», «Полная схема за 149 ₽», «149 ₽ один раз»,
   «Перейти к форме расчёта — 149 ₽»).
4. `configure/index.html` — meta-description.

В БД цены нет. `orders.base_price_kop` — это снапшот на момент создания
заказа (чтобы при историческом анализе цена не «уплыла»), но не
источник правды.

**Проблема:** при смене цены приходится править руками 4 файла +
.env на проде. Высокий риск рассинхрона — на лендинге одно, на
checkout-странице ЮКассы другое.

**Что нужно:**

1. Таблица `pricing` в БД (или конфиг-таблица `settings` c key/value),
   где лежит текущая активная цена + история изменений (`valid_from`,
   `valid_until`).
2. Эндпойнт `GET /api/pricing/current` → `{ amount_kop, formatted: "149 ₽" }`.
3. Лендинг подтягивает цену через `<script>` при загрузке страницы
   (с graceful fallback на захардкоженный текст, если эндпойнт упал —
   чтобы CTA-кнопки не сломались).
4. Бэк (calculate.ts) читает цену из БД вместо `env.PRICE_KOP`. Сам
   `PRICE_KOP` из .env можно выпилить.
5. Оферта остаётся «бумажным» документом — п. 5.1 по-прежнему правится
   руками при смене цены, потому что это юридический текст и его
   автоматизировать нельзя (нужна явная редакция с датой публикации).

**Когда делать:** после первых 10-20 реальных платежей, когда станет
понятно, что цена 149 ₽ устаканилась. Если будем экспериментировать
с ценой (199/249/99 для разных сегментов), приоритет повышается —
без single source of truth A/B-тесты по цене делать опасно.
...
### BL-17: Сохранение ввода формы /configure/ при перезагрузке

**Status:** open — UX-оптимизация. Подозреваемый источник просадки
конверсии: пользователь случайно жмёт F5 / возвращается «назад» из
ЮКассы / переключается с вкладки и теряет фокус → форма очищается →
заполнять заново лень → ушёл. Чем длиннее форма (сейчас ~7 полей:
размеры, категории, объёмы, приоритет, согласие), тем больнее.

**Контекст:** Сейчас все поля .u-calc хранятся только в DOM. На
перезагрузке всё сбрасывается. Заполнено пользователем — не
сохраняется. Особенно болезненно после оплаты, когда юзер
возвращается с YooKassa на наш домен — `/result/` загружен через
поллинг, но если он по ошибке зашёл назад на `/configure/`, форма
пустая.

**Что нужно:**

1. **Хранилище:** `localStorage` под ключом `umestno.configure.draft` —
   JSON с полями формы (размеры, items[], priority, consent_oferta).
   Email НЕ сохраняем (152-ФЗ — ПДн без согласия не храним).
2. **Сохранение:** debounced (~300ms) `input` event-listener на
   родительском контейнере формы. На каждое изменение — `JSON.stringify`
   → `localStorage.setItem(...)`. Старая запись затирается.
3. **Восстановление:** при загрузке `/configure/` (DOMContentLoaded) —
   читаем `localStorage.getItem(...)`, если есть валидный JSON и не
   старше N дней (например 7) — раскладываем по полям. Иначе игнорируем.
4. **Очистка после успешного сабмита:** в `submitCalculation()` после
   получения `token` от сервера — `localStorage.removeItem(...)`. Иначе
   при следующем заходе подсунем уже устаревший расчёт.
5. **TTL:** записи старше 7 дней игнорируем при восстановлении (для
   гигиены — старые черновики юзеру не нужны).
6. **GDPR/152-ФЗ:** localStorage технически — ПДн? Нет, если там нет
   email/телефона/имени. Размеры ящика + категории вещей — анонимные
   технические параметры. Так что без отдельного согласия можно.
7. **Не сохраняем:**
   - Поле email (требует согласия)
   - Поле промокода (одноразовое применение, не имеет смысла
     сохранять — пользователь может ввести заново, если хочет)
   - Чекбокс «принимаю оферту» (согласие — единичный акт)

**Метрика для оценки эффекта:** добавить goal `draft_restored` — стреляет
один раз при успешном восстановлении из localStorage. После релиза
сравнить конверсию визит → calc_submitted у юзеров с
`draft_restored=true` vs без.

**Что НЕ делаем:**

- Не используем cookies (тяжелее localStorage + отправляются на сервер
  с каждым запросом — лишний трафик)
- Не синхронизируем между устройствами (overkill для MVP)
- Не сохраняем после клика на кнопку «Получить расчёт» — это уже
  состояние «отправлено», черновик больше не нужен

**Когда делать:** после релиза, как только первые цифры конверсии
покажут реальную просадку на этапе «зашёл на /configure/ → нажал
Получить расчёт». Если разрыв >50% — приоритет 1.

**Зависимости:** нет.

### BL-18: Cells-layout библиотека — добавить «вытянутые» раскладки

**Status:** ✓ closed 2026-06-10. Реализовано так:
- В `defaultLibraries.zoneLayoutOptions` добавлены вытянутые/крупные
  cells-раскладки: `cells_3x5` (15), `cells_3x6` (18), `cells_4x6` (24),
  `cells_4x7` (28) и `cells_7x8` (56, закрыл разрыв по вместимости для
  jewelry_small Много = 50, где раньше максимум был 30 ячеек → no_scheme).
- Полная факторизация для ВСЕХ count'ов cells-категорий: для каждого
  количества предметов (2,3,4,6,7,8,10,12,15,16,24,25,50) в библиотеке есть
  все неполосные прямоугольные сетки (стороны ≥2, aspect ≤6). Добавлены
  «спасательные» сетки `cells_2x4` (=8), `cells_2x5` (=10), `cells_2x6` (=12),
  `cells_2x8` (=16), `cells_3x8`/`cells_2x12` (=24), `cells_5x10` (=50).
  Простые/малые count'ы (2, 3, 7) имеют только вырожденные `1×N`-полосы —
  их обслуживает ближайшая квадратная сетка. count 18 ни одна категория
  не порождает. Полнота проверяется авто-аудитом в `runCalibration.ts`.
  Они равны компактным 4×4/4×6 по surplus, поэтому вынесены в `FALLBACK_CELLS`
  и подключаются **только когда ни одна компактная сетка (даже повёрнутая) не
  вписывается в геометрию ящика** — так очень узкие/глубокие ящики получают
  схему, а обычные ящики сохраняют привычные компактные раскладки. Исключены
  из geometry-blind пути `generateCalculatedZones`, чтобы не «утекать» в дефолт.
  Принцип: движок выбирает форму под геометрию ящика, фиксированного «дефолта» нет.
- `count_min/count_max` подобраны так, чтобы вытянутые опции не перебивали
  квадратные в обычных ящиках (3×6 имеет больший surplus, чем 4×4, поэтому
  4×4 остаётся дефолтом и переключается на 3×6 только когда не лезет).
  Исключение — socks Много (24): `cells_4x6` имеет точную вместимость 24
  против 25 у `cells_5x5`, поэтому становится дефолтом (это корректнее —
  ровно 24 ячейки без потерь).
- В `generateZoneVariants.ts` добавлен **fit-aware поворот cells**: повёрнутый
  вариант сетки добавляется только если прямая ориентация не лезет в ящик,
  а повёрнутая лезет. Это сохраняет байт-в-байт все существующие схемы,
  которые уже помещались, и разблокирует вытянутые раскладки в неглубоких
  ящиках (socks `cells_4x6` → 37×25; jewelry_large `cells_3x5` → 40×24;
  ties `cells_3x4` → 32×24).
- Калибровка: все прежние кейсы зелёные + 7 новых сценариев на
  «маленький ящик + средние/большие объёмы» (socks/panties/ties/jewelry_small,
  мульти-категория, граничный no_scheme-guard, backward-compat большого ящика)
  в `engine/test/runCalibration.ts`.
- Граничный случай: ящик 35×30 для socks Много остаётся `no_scheme` —
  единственная подходящая сетка (4×6 повёрнутая 37×25) требует ширину 37 см.
  Это честный физический предел, не баг.

Старое описание сохраняю для истории:

**Реальный кейс** провалился (40×30×20 ящик, носки Много 24 пары) — движок
не смог найти раскладку, хотя интуитивно носки должны были влезть.

**Контекст:** Сейчас `defaultLibraries.zoneLayoutOptions` для
division_type=`cells` содержит только «квадратные» раскладки:

```
cells_4x4  → 16 ячеек (count_min: 13, count_max: 16)
cells_4x5  → 20 ячеек (count_min: 17, count_max: 20)
cells_5x5  → 25 ячеек (count_min: 21, count_max: 25)
```

Для носков Много (24 пары) подходит ТОЛЬКО `cells_5x5` (квадратная
сетка). Зона получается 31×31 см. Поворот не помогает (квадрат),
open-фоллбэк по какой-то причине не выбирается. В ящике 40×30
зона не лезет → no_scheme.

Если бы в библиотеке была опция **`cells_4x6` (24 ячейки)**:
- Зона была бы ~25×37 см
- Движок имеет `can_rotate: true` → перевернул бы до 37×25
- Это влезло бы в 40×30 ящик
- → fit_all вместо no_scheme

**Что нужно добавить:**

1. **Вытянутые cells-раскладки** для всех ячеек-категорий:
   - `cells_3x4` (12 ячеек, count_max: 12)
   - `cells_4x6` (24 ячеек, count_max: 24)
   - `cells_3x8` (24 ячеек — вариант — длинный)
   - `cells_4x7` (28 ячеек)
   - возможно: `cells_3x5`, `cells_3x6`, `cells_2x8`

2. **Грамотные пересечения по count_min/count_max** — сейчас интервалы
   жёсткие (13-16, 17-20, 21-25). Нужно проверить чтобы новые опции
   не пересекались некорректно с существующими.

3. **Перекалибровать выбор кандидата** — у нас сейчас движок выбирает
   лучшего кандидата через selectBestCandidate. Добавление новых
   опций может изменить выбор для существующих fit_all-кейсов. Нужно
   прогнать calibration suite и убедиться что ничего не сломалось.

4. **Проверить остальные категории на ту же проблему:**
   - panties (Много 16) — есть `cells_4x4`, должно работать
   - tights (Много 8) — какие есть?
   - sport_tops, bras — slots, не cells, не затронуто
   - другие? — пройтись по всем category × volume_level комбинациям
     и убедиться что для каждой есть подходящая раскладка с разумными
     пропорциями

**Тесты:** добавить test-cases с маленькими ящиками + средне-большими
объёмами категорий. Сейчас calibration в основном на больших ящиках,
маленькие плохо покрыты.

**Когда делать:** после релиза, при анализе соотношения no_scheme
к fit_all/fit_partial. Если no_scheme занимает >10% от всех расчётов,
это критично для конверсии — приоритет 1. Иначе можно отложить.

**Зависимости:** связано с BL-04 (volumeToCount mismatch) и BL-11
(fit_partial UX) — все три связаны с покрытием реальных кейсов.
