...
**Status:** open — текущее поведение временное, нужен продуктовый
дизайн до Стадии 3.

**Текущее поведение (после фикса 02.06):** при `fit_partial` юзера
редиректят на `/no-fit/` со стандартным текстом «не подошло».
...
4. Связь с оплатой (Стадия 3): на `fit_partial` мы **не** продаём
   PDF/email; но free-режим может показывать что-то полезное?
   (например, схема того что влезло — без партнёрских ссылок?)
5. Email-капча: оставлять кнопку «прислать вариант» на /no-fit/
