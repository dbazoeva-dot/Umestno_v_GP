...
`can_split` in the MVP means **internal split inside one contiguous assigned zone**. It is equivalent to an internal-layout capability, not permission to scatter one category around the drawer.

Valid `can_split` behavior examples:

- multiple parallel slot lanes inside the same bras zone;
- multiple rows/columns inside the same cells or open zone;
- choosing an alternative internal layout option for one content type.

Default MVP rule: **one `content_type` produces one contiguous assigned zone**. The engine must not place the same content type into separate non-contiguous drawer areas unless a future explicit field such as `can_spatial_split = true` is introduced. For the current MVP, treat `can_spatial_split` as `false` for all content types.

Therefore, `bras` multi-lane slot splitting is a valid `can_split` use because it remains one contiguous bras zone. It should be described as internal slot-lane splitting, not as splitting bras into separate zones across the drawer.
