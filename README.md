...
```
blog/
  index.html                            — листинг журнала (masthead + featured + сетка + CTA)
  _template/index.html                  — шаблон-пример для новой статьи (новый дизайн)
  kak-navesti-poryadok-v-komode/index.html — пример готовой статьи
```
...
2. Открой `index.html` в новой папке. Все места, которые нужно заменить, помечены `[…]` — пройдись по ним и подставь:
   ...
   - рубрику, заголовок (слово в `<em>` получит фирменное подчёркивание), лид (`.p-lede`), превью (`.p-masthead__preview`), дату и время чтения (`.p-byline`);
...
### Сигнатурные блоки статьи (готовые в шаблоне)

- **`.p-pull`** — крупная цитата-врезка с акцентным кантом слева.
- **`.p-pairs`** (`.p-pair--good` / `.p-pair--care`) — две колонки «Хорошо вместе / С осторожностью».
- **`.p-note`** — короткое предупреждение «!» на тёплой плашке.
- **`.p-figure`** — картинка в тексте с моно-подписью.
- **`.p-mistakes`** — нумерованный анти-список «Частые ошибки».
- **`.p-cta`** — врезка с ценой 149 ₽ и кнопкой на конфигуратор (одна на статью).

Плюс: дроп-капс на первом абзаце, заголовки `h2` с моно-нумерацией, общий хедер/футер сайта,
Open Graph + JSON-LD `Article`, липкое содержание со scroll-spy.

Хочешь подкрутить стиль — всё в `landing_design/blog.css` (классы `j-*` — листинг, `p-*` — статья).
