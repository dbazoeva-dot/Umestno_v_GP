...
export async function GET(
  ...
  const row = await queryOne<{
    ...
    result_full: {
      result: {
        scheme: { assigned_zones: PlacedZone[]; warnings: string[]; content_warnings: SoftHeightWarning[] };
    ...
    `SELECT o.status, o.variant, c.result_full, c.input
     FROM orders o JOIN configurations c ON c.id = o.configuration_id
  ...
  const pdfBuffer = await generatePdf({
    ...
    drawerSize: { w_cm: row.input.drawer_width_cm, d_cm: row.input.drawer_depth_cm, h_cm: row.input.drawer_height_cm },
