// Builds the unified scheme_zones view model (Option A)
// Used by both the API route and the preview script
...
// The engine emits many overlapping maximal-free-rectangles (bin-packing artifact).
// We pick the single largest non-trivial rectangle as the visual reserve zone.
function selectBestReserveRect(
  rects: FreeRect[],
  drawerW: number,
  drawerD: number
): FreeRect | null {
  const minArea = drawerW * drawerD * 0.06; // must be > 6% of drawer area
  const sorted = rects
